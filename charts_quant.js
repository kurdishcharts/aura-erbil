// TradingView-style quantitative analytics – 24 charts + social + indicator toggles
(function () {
  'use strict';
  ...
  function buildAnalytics(articles) {
    ...
    // B3. Weekday Activity
    {
      const card   = mkCard(panel, 'Weekday Activity');
      const canvas = mkCanvas(card, 145);
      const dow    = new Array(7).fill(0);
      articles.forEach(a => { if (a.timestamp) dow[new Date(a.timestamp).getDay()]++; });
      mkC(canvas, {
        type: 'bar',
        data: { labels: ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'], datasets: [{ data: dow, backgroundColor: '#e0e7ff', borderWidth: 0, borderRadius: 5 }] },
        options: { ...BASE, scales: { x: { ticks: TICK, grid: { display: false } }, y: { ticks: TICK, grid: { color: GRID }, beginAtZero: true } } }
      });
    }
    ...
    // B4. Source × Sentiment
    {
      const card   = mkCard(panel, 'Source × Sentiment');
      const canvas = mkCanvas(card, 145);
      const srcs   = [...new Set(articles.map(a => a.source).filter(Boolean))].slice(0, 8);
      mkC(canvas, {
        type: 'bar',
        data: { labels: srcs, datasets: [
          { label: 'Positive', data: srcs.map(s => articles.filter(a => a.source===s && a.sentiment==='positive').length), backgroundColor: 'rgba(34,197,94,.75)' },
          { label: 'Negative', data: srcs.map(s => articles.filter(a => a.source===s && a.sentiment==='negative').length), backgroundColor: 'rgba(239,68,68,.75)' },
          { label: 'Neutral',  data: srcs.map(s => articles.filter(a => a.source===s && a.sentiment==='neutral').length),  backgroundColor: 'rgba(245,158,11,.65)' },
        ]},
        options: { ...BASE,
          plugins: { legend: { display: true, position: 'top', labels: { font: { family: 'Inter', size: 9 }, color: '#787b86', padding: 8, usePointStyle: true, pointStyleWidth: 6 } } },
    ...
    {
      const card    = mkCard(panel, 'Silence Detection · 48h');
      const canvas  = mkCanvas(card, 145);
      const hourMap = {};
      articles.forEach(a => {
        if (!a.timestamp) return;
      ...
      mkC(canvas, {
        type: 'bar',
        data: { labels: last48L, datasets: [{ data: last48, backgroundColor: last48.map(v => v === 0 ? 'rgba(239,68,68,.45)' : 'rgba(34,197,94,.45)'), borderWidth: 0, borderRadius: 2 }] },
    ...
    {
      const card     = mkCard(panel, 'Sentiment Volatility · 5d STD');
      const canvas   = mkCanvas(card, 145);
      ...
      addIndicatorToggles(card, chartObj, rollStd.filter(v=>v!==null));
    }

    // C2. Sentiment Momentum
    {
      const card     = mkCard(panel, 'Sentiment Momentum');
      const canvas   = mkCanvas(card, 145);
      ...
      mkC(canvas, {
        type: 'bar',
        data: { labels: shortLbls, datasets: [{ data: mom, backgroundColor: mom.map(v=>v>=0?'rgba(34,197,94,.65)':'rgba(239,68,68,.65)'), borderColor: mom.map(v=>v>=0?'#22c55e':'#ef4444'), borderWidth: 1, borderRadius: 3 }] },
    ...
    // C3. Panic Index
    {
      const card = mkCard(panel, 'Panic Index');
      ...
      num.style.cssText = `text-align:center;padding:12px 0 2px;font-family:'JetBrains Mono',monospace;font-size:42px;font-weight:500;color:${col};line-height:1;`;
      num.textContent = pct+'%'; card.appendChild(num);
      const lb = document.createElement('div');
      lb.style.cssText = `text-align:center;font-size:10px;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:${col};margin-bottom:10px;`;
      lb.textContent = lbl; card.appendChild(lb);
      const canvas = mkCanvas(card, 65);
      mkC(canvas, { type:'doughnut', data:{ datasets:[{ data:[pct,100-pct], backgroundColor:[col,'#2a2e39'], borderWidth:0, circumference:180, rotation:270 }]}, options:{...BASE,cutout:'65%',plugins:{legend:{display:false},tooltip:{enabled:false}}} });
    }
    ...
    {
      const card   = mkCard(panel, 'Category Volatility · STD');
      ...
      mkC(canvas, {
        type: 'radar',
        data: { labels: cats, datasets: [{ data: catStd, borderColor: '#2962ff', backgroundColor: 'rgba(41,98,255,.1)', pointBackgroundColor: '#2962ff', pointRadius: 3, borderWidth: 1.5 }] },
        options: { ...BASE, scales: { r: { ticks:{...TICK,backdropColor:'transparent',maxTicksLimit:3}, grid:{color:GRID}, angleLines:{color:GRID}, pointLabels:{font:{family:'Inter',size:9},color:'#787b86'}, min:0 } } }
      });
    }

    // C6. Intraday Swing
    {
      const card   = mkCard(panel, 'Intraday Swing');
      const canvas = mkCanvas(card, 145);
      ...
      mkC(canvas, {
        ...
        data: { labels: shortLbls, datasets: [
          { label:'High',  data:highs, borderColor:'#22c55e', backgroundColor:'rgba(34,197,94,.07)',  fill:'+1', tension:.3, pointRadius:1, borderWidth:1.5 },
          { label:'Low',   data:lows,  borderColor:'#ef4444', backgroundColor:'rgba(239,68,68,.07)', fill:false, tension:.3, pointRadius:1, borderWidth:1.5 },
          { label:'Close', data:close, borderColor:'#2962ff', backgroundColor:'transparent',          fill:false, tension:.3, pointRadius:2, borderWidth:2, borderDash:[4,3] },
        ]},
        options: { ...BASE,
          plugins: { legend:{ display:true, position:'top', labels:{ font:{family:'Inter',size:9}, color:'#787b86', padding:8, usePointStyle:true, pointStyleWidth:6 } } },
    ...
    // D1. Entity Type Distribution
    {
      ...
      const colors = ['#2962ff','#22c55e','#7c3aed','#f59e0b','#ef4444','#0891b2','#be185d','#0f766e'];
      mkC(canvas, {
        type: 'doughnut',
        data: { labels: types, datasets: [{ data: types.map(t=>typeCounts[t]), backgroundColor: colors.slice(0,types.length), borderWidth: 0 }] },
        options: { ...BASE, cutout:'60%', plugins:{ legend:{ display:true, position:'right', labels:{ font:{family:'Inter',size:9}, color:'#787b86', padding:6, usePointStyle:true, pointStyleWidth:6 } } } }
      });
    }
    ...
    // D2. Top Persons Mentioned
    {
      ...
      mkC(canvas, {
        type: 'bar',
        data: { labels: top.map(t=>t[0]), datasets: [{ data: top.map(t=>t[1]), backgroundColor: 'rgba(41,98,255,.65)', borderWidth: 0, borderRadius: 4 }] },
    ...
    // D3. Top Organizations
    {
      ...
      mkC(canvas, {
        type: 'bar',
        data: { labels: top.map(t=>t[0]), datasets: [{ data: top.map(t=>t[1]), backgroundColor: 'rgba(124,58,237,.65)', borderWidth: 0, borderRadius: 4 }] },
        options: { ...BASE, indexAxis:'y', scales: { x:{ticks:TICK,grid:{color:GRID},beginAtZero:true}, y:{ticks:{...TICK,font:{size:9}},grid:{display:false}} } }
      });
    }
    ...
    // D4. Entity Co-occurrence
    {
      ...
      mkC(canvas, {
        type: 'bar',
        data: { labels: top.map(t=>t[0]), datasets: [{ data: top.map(t=>t[1]), backgroundColor: 'rgba(8,145,178,.65)', borderWidth: 0, borderRadius: 3 }] },
    ...
    // D5. Entity Sentiment Breakdown
    {
      ...
      mkC(canvas, {
        ...
        data: { labels: top.map(t=>t[0]), datasets:[
          { label:'Pos', data:top.map(t=>t[1].pos), backgroundColor:'rgba(34,197,94,.75)',  borderWidth:0 },
          { label:'Neg', data:top.map(t=>t[1].neg), backgroundColor:'rgba(239,68,68,.75)',  borderWidth:0 },
          { label:'Neu', data:top.map(t=>t[1].neu), backgroundColor:'rgba(245,158,11,.65)',  borderWidth:0 },
        ]},
        options: { ...BASE,
          plugins:{ legend:{ display:true, position:'top', labels:{font:{family:'Inter',size:9},color:'#787b86',padding:6,usePointStyle:true,pointStyleWidth:6} } },
    ...
    // E1. Sentiment Z-Score
    {
      ...
      mkC(canvas, {
        type: 'bar',
        data: { labels: shortLbls, datasets: [{ data: zScores, backgroundColor: zScores.map(v=>v>=0?'rgba(34,197,94,.65)':'rgba(239,68,68,.65)'), borderWidth:0, borderRadius:3 }] },
        options: { ...BASE, scales: { x:{ticks:TICK,grid:{display:false}}, y:{ticks:TICK,grid:{color:GRID}} } }
      });
    }
    ...
    // E2. Category Correlation
    {
      ...
      mkC(canvas, {
        type: 'bar',
        data: { labels: cats, datasets: [{ data: pctPos, backgroundColor: pctPos.map(v=>v>=50?'rgba(34,197,94,.65)':'rgba(239,68,68,.65)'), borderWidth:0, borderRadius:4 }] },
    ...
    // E3. Bull/Bear Acceleration
    {
      ...
      mkC(canvas, {
        type: 'bar',
        data: { labels: shortLbls, datasets: [{ data: accel, backgroundColor: accel.map(v=>v>=0?'rgba(34,197,94,.65)':'rgba(239,68,68,.65)'), borderWidth:0, borderRadius:3 }] },
        options: { ...BASE, scales: { x:{ticks:TICK,grid:{display:false}}, y:{ticks:TICK,grid:{color:GRID}} } }
      });
    }
    ...
    // E4. Sentiment Velocity · Hourly
    {
      ...
      addIndicatorToggles(card, chartObj, hourAvg.filter(v=>v!==null));
    }
    ...
    // E6. Daily Volatility
    {
      ...
      mkC(canvas, {
        type:'bar',
        data: { labels: shortLbls, datasets: [{ data: swing, backgroundColor:'rgba(124,58,237,.55)', borderWidth:0, borderRadius:3 }] },
        options: { ...BASE, scales: { x:{ticks:TICK,grid:{display:false}}, y:{ticks:TICK,grid:{color:GRID},beginAtZero:true} } }
      });
    }
    ...
    // E7. SMA Crossover
    {
      ...
      const canvas   = mkCanvas(card, 145);
      const dailyAvg = days.map(d => { const arr = byDay[d].map(sentScore); return arr.reduce((a,b)=>a+b,0)/arr.length; });
      const smaCalc  = n => dailyAvg.map((_,i) => i<n-1 ? null : +(dailyAvg.slice(i-n+1,i+1).reduce((a,b)=>a+b,0)/n).toFixed(3));
      mkC(canvas, {
        type:'line',
        data: { labels: shortLbls, datasets: [
          { label:'SMA3', data:smaCalc(3), borderColor:'#2962ff', tension:.4, pointRadius:1.5, borderWidth:2,   spanGaps:true },
          { label:'SMA7', data:smaCalc(7), borderColor:'#f59e0b', tension:.4, pointRadius:1.5, borderWidth:1.5, borderDash:[4,3], spanGaps:true },
        ]},
        options: { ...BASE,
          plugins:{ legend:{ display:true, position:'top', labels:{font:{family:'Inter',size:9},color:'#787b86',padding:8,usePointStyle:true,pointStyleWidth:6} } },
    ...
    // E8. Intraday Sentiment Swing · 4h
    {
      ...
      mkC(canvas, {
        type:'bar',
        data: { labels: blocks, datasets: [{ data: bData, backgroundColor: bData.map(v=>v>=0?'rgba(34,197,94,.65)':'rgba(239,68,68,.65)'), borderWidth:0, borderRadius:4 }] },
        options: { ...BASE, scales: { x:{ticks:TICK,grid:{display:false}}, y:{ticks:TICK,grid:{color:GRID},min:-1,max:1} } }
      });
    }
    ...
    // F1. Flashpoint Detection
    {
      ...
      mkC(canvas, {
        type: 'bar',
        data: { labels: shortLbls, datasets: [
          { label: 'Z‑Score', data: zVol, backgroundColor: zVol.map(v => v>=2?'rgba(239,68,68,.85)':v>=1?'rgba(245,158,11,.65)':'rgba(41,98,255,.40)'), borderWidth:0, borderRadius:4 },
          { label: 'Spike', data: highlights, backgroundColor: 'rgba(239,68,68,.90)', borderWidth:0, type:'bar', order:1 }
        ]},
        options: { ...BASE, scales: { x:{ticks:TICK,grid:{display:false}}, y:{ticks:TICK,grid:{color:GRID}} } }
      });
    }
    ...
    // F2. Polarization Index
    {
      ...
      mkC(canvas, {
        type: 'bar',
        data: { labels: bins.map(b => b.toFixed(1)), datasets: [{ data: hist, backgroundColor: hist.map((_,i) => i<5?'rgba(239,68,68,.65)':'rgba(34,197,94,.65)'), borderWidth:0, borderRadius:3, barPercentage:0.95 }] },
    ...
    // F3. Source Bias Matrix
    {
      ...
      mkC(canvas, {
        type: 'scatter',
        data: { datasets: [{ label: 'Sources', data: data, backgroundColor: data.map(d => d.x >= 0 ? 'rgba(34,197,94,.7)' : 'rgba(239,68,68,.7)'), borderColor: 'transparent' }] },
        options: { ...BASE,
    ...
    // F4. Political Weight Bar
    {
      ...
      mkC(canvas, {
        type: 'bar',
        data: { labels: top.map(t => t[0]), datasets: [{ data: top.map(t => t[1]), backgroundColor: 'rgba(41,98,255,.70)', borderWidth:0, borderRadius:3 }] },
    ...
    // F5. Stability Index
    {
      ...
      const color      = stability >= 60 ? '#22c55e' : stability >= 40 ? '#f59e0b' : '#ef4444';
      mkC(canvas, {
        type: 'doughnut',
        data: { datasets: [{ data: [stability, 100-stability], backgroundColor: [color, '#2a2e39'], borderWidth:0, circumference:180, rotation:270 }] },
    ...
    // F6. Reach vs Impact Bubble
    {
      ...
      mkC(canvas, {
        type: 'bubble',
        data: { datasets: [{ data: data, backgroundColor: data.map(d => d.sentiment >= 0 ? 'rgba(34,197,94,.6)' : 'rgba(239,68,68,.6)'), borderColor: 'transparent' }] },
    ...
    // F7. Source Reliability
    {
      ...
      mkC(canvas, {
        ...
        data: { labels: data.map(d => d.src), datasets: [
          { label:'Fact', data: data.map(d => d.fact), backgroundColor:'rgba(41,98,255,.75)', borderWidth:0 },
          { label:'Opinion', data: data.map(d => d.op), backgroundColor:'rgba(239,68,68,.55)', borderWidth:0 }
        ]},
        options: { ...BASE,
          plugins:{ legend:{ display:true, position:'top', labels:{font:{family:'Inter',size:9},color:'#787b86',padding:6,usePointStyle:true,pointStyleWidth:6} } },
    ...
    // G1. Article Velocity 7‑Day
    {
      ...
      mkC(canvas, {
        ...
        data: { labels: shortLbls, datasets: [
          ...
          { label: '7D Avg', data: rolling, borderColor: '#2962ff', borderWidth: 2.5, tension: .4, pointRadius: 2, pointBackgroundColor: '#2962ff', spanGaps: true }
        ]},
        options: { ...BASE,
          plugins: { legend: { display: true, position: 'top', labels: { font: {family:'Inter',size:9}, color:'#787b86', padding:6, usePointStyle:true, pointStyleWidth:6 } } },
    ...
    // G2. Sentiment Volatility Bands
    {
      ...
      mkC(canvas, {
        ...
        data: { labels: shortLbls, datasets: [
          ...
          { label: 'Lower', data: lower, borderColor: 'transparent', backgroundColor: 'rgba(41,98,255,0.08)', fill: 1, pointRadius: 0 },
          { label: 'Sentiment', data: dailyAvg, borderColor: '#f59e0b', borderWidth: 2, tension: .4, pointRadius: 2, pointBackgroundColor: '#f59e0b' }
        ]},
        options: { ...BASE,
    ...
    // G3. Source Dominance Area
    {
      ...
      const colors = ['#2962ff','#f59e0b','#ef4444','#22c55e','#7c3aed','#0891b2'];
      mkC(canvas, {
        type: 'bar',
        data: { labels: shortLbls, datasets: topSources.map((s,i) => ({
          label: s, data: days.map(d => byDaySource[d][s]||0), backgroundColor: colors[i%6], borderWidth: 0, borderRadius: 2
        }))},
        options: { ...BASE,
          plugins: { legend: { display: true, position: 'top', labels: { font: {family:'Inter',size:8}, color:'#787b86', padding:4, usePointStyle:true, pointStyleWidth:4 } } },
    ...
    // G4. Geographical Impact
    {
      ...
      mkC(canvas, {
        type: 'bar',
        data: { labels: topCities.map(e => e[0]), datasets: [{ data: topCities.map(e => e[1]), backgroundColor: 'rgba(34,197,94,.65)', borderWidth:0, borderRadius:4 }] },
        options: { ...BASE, indexAxis:'y', scales: { x:{ticks:TICK,grid:{color:GRID}}, y:{ticks:{...TICK,font:{size:8}},grid:{display:false}} } }
      });
    }
    ...
    // G5. Thematic Sentiment Radar
    {
      ...
      mkC(canvas, {
        type: 'radar',
        data: { labels: categories, datasets: [{ data: categorySent, borderColor: '#f59e0b', backgroundColor: 'rgba(245,158,11,.1)', borderWidth: 2, pointBackgroundColor: '#f59e0b' }] },
        options: { ...BASE,
          plugins: { legend: { display: false } },
  ...
  // ═══ buildSocialAnalytics – TikTok charts ═══
  function buildSocialAnalytics(articles) {
    ...
    // F1 – Posts per Period
    {
      const card = mkCard(panel, 'TikTok Posts per Period');
      const canvas = mkCanvas(card, 145);
      const byDay = {};
      tiktokArts.forEach(a => {
        if (!a.timestamp) return;
        const d = a.timestamp.slice(0,10);
        byDay[d] = (byDay[d]||0)+1;
      });
      ...
      mkC(canvas, {
        type: 'bar',
        data: { labels: lbls, datasets: [{ data: vals, backgroundColor: '#fe2c55' }] },
        options: { ...BASE, scales: { x:{ticks:TICK,grid:{display:false}}, y:{ticks:TICK,grid:{color:GRID},beginAtZero:true} } }
      });
    }
    ...
    // F2 – Topic Heatmap
    {
      const card = mkCard(panel, 'Topic Heatmap · Accounts × Categories');
      const accounts = getAccounts();
      const categories = [...new Set(tiktokArts.map(a => a.category).filter(Boolean))].sort();
      const grid = document.createElement('div');
      grid.style.cssText = 'display:grid;grid-template-columns:auto repeat('+categories.length+',1fr);gap:2px;font-size:9px;';
      grid.innerHTML = '<div></div>';
      categories.forEach(c => { const d=document.createElement('div'); d.textContent=c; d.style.cssText='font-weight:600;padding:2px 4px;'; grid.appendChild(d); });
      accounts.forEach(acc => {
        const accDiv = document.createElement('div');
        accDiv.textContent = acc.replace('tiktok-','').slice(0,12);
        accDiv.style.cssText = 'font-weight:600;padding:2px 4px;';
        grid.appendChild(accDiv);
        categories.forEach(cat => {
          const cnt = tiktokArts.filter(a => a.source===acc && a.category===cat).length;
          const cell = document.createElement('div');
          cell.textContent = cnt || '';
          cell.style.cssText = `background:rgba(254,44,85,${Math.min(cnt/10,0.9)})` + (cnt===0 ? 'background:#1e222d;' : '') + 'padding:2px 4px;text-align:center;';
    ...
    // F3 – Propaganda Level
    {
      const card = mkCard(panel, 'Pro‑Kurdish Propaganda Level (simulated)');
      const canvas = mkCanvas(card, 145);
      const accounts = getAccounts();
      const vals = accounts.map(() => Math.floor(Math.random()*100));
      mkC(canvas, {
        type: 'bar',
        data: { labels: accounts, datasets: [{ data: vals, backgroundColor: vals.map(v => v>60?'#22c55e':'#f59e0b') }] },
        options: { ...BASE, scales: { x:{ticks:TICK,grid:{display:false}}, y:{ticks:TICK,grid:{color:GRID},beginAtZero:true,max:100} } }
      });
    ...
    // F4 – Crime chart
    {
      const card = mkCard(panel, 'Crime in Region (TikTok)');
      const canvas = mkCanvas(card, 145);
      ...
      mkC(canvas, {
        type: 'line',
        data: { labels: days.map(d=>d.slice(5)), datasets: [{ data: vals, borderColor: '#ef4444', fill:false, tension:.3 }] },
        options: { ...BASE, scales: { x:{ticks:TICK,grid:{display:false}}, y:{ticks:TICK,grid:{color:GRID},beginAtZero:true} } }
      });
    }
    ...
    // F5 – Leaderboard
    {
      const card = mkCard(panel, 'Fastest Reporter · Avg Delay (simulated)');
      const canvas = mkCanvas(card, 145);
      const accounts = getAccounts();
      const delays = accounts.map(() => Math.floor(Math.random()*120+1));
      mkC(canvas, {
        type: 'bar',
        data: { labels: accounts, datasets: [{ data: delays, backgroundColor: delays.map(d => d<30?'#22c55e':'#f59e0b') }] },
        options: { ...BASE, indexAxis:'y', scales: { x:{ticks:TICK,grid:{color:GRID},title:{display:true,text:'minutes'}}, y:{ticks:{...TICK,font:{size:8}},grid:{display:false}} } }
      });
      const note = document.createElement('div');
    ...
    // F6 – Posting Frequency per Account
    {
      const card = mkCard(panel, 'Posting Frequency per Account');
      const canvas = mkCanvas(card, 145);
      const accounts = getAccounts();
      const counts = accounts.map(acc => tiktokArts.filter(a => a.source===acc).length);
      mkC(canvas, {
        type: 'bar',
        data: { labels: accounts, datasets: [{ data: counts, backgroundColor: '#fe2c55' }] },
        options: { ...BASE, scales: { x:{ticks:{...TICK,maxRotation:45},grid:{display:false}}, y:{ticks:TICK,grid:{color:GRID},beginAtZero:true} } }
      });
    }
  }
